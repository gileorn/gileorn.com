...
![Medium subscription promotion](/img/posts/hello-world-devblog/medium.png 'Not going to invest my time for content that will end up looking like this.')
...
- Visually attractive, minimalistic UI with responsiveness, adapted for mobile phones
...
Tailwind was another thing I have yet to try. The utility classes concept was familiar to me, but I've only encountered some custom implementations of it and haven't seen any project taking full advantage of Tailwind.
...
So in terms of the code, this is what I ended up with. Below is the PostHeading component you see on the top of this page.
...
The above defined fields then can be initialized in the heading section of any mdx file as shown below.
...
Comments seemed like one of the most resource-consuming things to build if we're talking about MVP of a blog with limited use of the database. Giscus was such a relief for me when I first found it. Giscus wraps the GitHub Discussions API to provide a very convenient way to integrate comments and reactions for your blog with GitHub authentication. And there's a dedicated npm package for React that made the integration even easier.

![UI of the comments section](/img/posts/hello-world-devblog/giscus.jpg 'Illustration of how Giscus works. This widget is lazily loaded and initiaded once user has scrolled down to it. Under the hood everything works as a mirror to GitHub Discussions.')
